...
process.env.VIEW_PATH = process.cwd() + "/views/";
...
function Application(customKoa) {
	...
	// Setup view
	if (config.view) {
		c.green("\tSetup view:\t" + config.view.default);
		this.app.use(views(process.env.VIEW_PATH, config.view));
	} else {
		c.yellow("\tSetup view:\tOff");
	}

	if (config.server.public_path !== null) {
    c.green("\tStatic files:\t" + config.server.public_path);
  } else {
  	c.yellow("\tStatic files:\tOff");
  }

	// Setup Body Parser
	if (config.server.upload && config.server.upload.upload_path) {
		c.green("\tUpload files:\t" + config.server.upload.upload_path);
		config.server.upload.upload_path = process.cwd() + config.server.upload.upload_path;
		this.app.use(bodyParser(config.server.upload));
	} else {
		c.yellow("\tUpload files:\tOff");
	}
	...
	if (config.server.csrf) {
		require("koa-csrf")(this.app, config.server.csrf);
		c.green("\tCSRF:\t\tOn");
	} else {
		c.yellow("\tCSRF:\t\tOff");
	}
...
Application.initialiser = function() {
