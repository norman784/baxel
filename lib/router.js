...
let helper = function(before_action, action) {
  ...
  if (!before_action) return helpers;
  if (before_action.constructor === Array) {
    for (let i in before_action) {
      helpers = helpers.concat(helper(before_action[i], action));
    }
  } else if (before_action.only && before_action.only.constructor === Array && before_action.only.indexOf(action) === -1) return helpers;
  else if (before_action.only && before_action.only !== action) return helpers;
  else if (before_action.except && before_action.except.constructor === Array && before_action.except,indexOf(action) > -1) return helpers;
  else if (before_action.except && before_action.except === action) return helpers;

  if (before_action.helper.constructor === Array) {
    for (let i in before_action.helper) {
      helpers.push(process.env.HELPER_PATH + before_action.helper[i]);
    }
  } else {
    helpers.push(process.env.HELPER_PATH + before_action.helper);
  }
...
let mount = function(app, routes, multiple_domains) {
  ...
  for (let i in routes) {
    ...
    let str = sprintf("%-13s %-40s %s %s", routes[i].verb, routes[i].url, routes[i].controller, helpers.length > 0 ? ">> " + helpers : "");
...
module.exports = function(app) {
  ...
  if (domains.length == 1 || DOMAIN_MOUNT.length === 1) {
