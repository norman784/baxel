...
let helper = function(before_action, action) {
  ...
  if (before_action.constructor === Array) {
    for (let i in before_action) {
      ...
      before_action.only && 
      (
        (_.isArray(before_action.only) && before_action.only.indexOf(action) === -1) ||
        (!_.isArray(before_action.only) && before_action.only !== action)
      )
    ) {
    return result;
  } else if (
      ...
      (
        (_.isArray(before_action.except) && before_action.except.indexOf(action) === -1) ||
        (!_.isArray(before_action.except) && before_action.except !== action)
      )
    ) {
    return result;
  } else {
    if (_.isArray(before_action.helper)) {
  ...
  if (domains.length == 1 || DOMAIN_MOUNT.length === 1 || (domains - DOMAIN_IGNORE.length) === 1) {
    ...
    for (let k in domains) {
      ...
      if (config.server.seamless !== null && config.server.seamless !== undefined && config.server.seamless === false) {
        ...
        app.vhosts[domain] = vapp.listen(port);
      } else {
