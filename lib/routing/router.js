...
var router = require('koa-router')()
	...
	, _console = require('color-console')
	...
	, resources = function(url, ctrl, exclude) {
		...
		if (exclude.indexOf('save') == -1) {
			setRoute(url + '/new', ctrl + '#save', 'POST');
			setRoute(url + '/:id/edit', ctrl + '#save', 'PUT');
		}
	...
	, getController = function (options) {
		...
		try {
			...
			_console.red('\t\t' + options.method + '\t' + options.url + '\t Failed to load controller "' + action[0] + '" function "' + action[1] + '"');
		} catch (e) {
			_console.red('\t\t' + options.method + '\t' + options.url + '\t Failed to load controller "' + action[0] + '"');
		}
	...
	, setRoute = function (url, ctrl, method) {
		...
		_console.green('\t\t' + method + '\t' + url + '\t' + ctrl);
...
module.exports = function () {
	...
	for (var i in routes) {
		...
		if (!_.isArray(_route)) {
			_console.red('\t\t- Cannot load resources from route file: ' + options.configPath + '/routes/' + file);
			return;
		}
